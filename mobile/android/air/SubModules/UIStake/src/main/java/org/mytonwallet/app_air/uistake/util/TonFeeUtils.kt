...
data class TonOperationFees(val gas: BigInteger, val real: BigInteger)
...
val TON_GAS = mapOf(
    ...
    TonGasOperation.CLAIM_JETTONS to
        JettonStakingGas.JETTON_TRANSFER + JettonStakingGas.SIMPLE_UPDATE_REQUEST,
...
fun getTonOperationFees(operation: TonGasOperation): TonOperationFees = TonOperationFees(
    gas = ((TON_GAS[operation] ?: 0L) + DEFAULT_FEE).toBigInteger(),
    real = (TON_GAS_REAL[operation] ?: 0L).toBigInteger()
)

fun getTonStakingFees(type: String?): Map<String, TonOperationFees> = when (type) {
    "nominators" -> mapOf(
