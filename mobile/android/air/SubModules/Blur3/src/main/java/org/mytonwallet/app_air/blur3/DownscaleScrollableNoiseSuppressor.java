package org.mytonwallet.app_air.blur3;
...
import org.mytonwallet.app_air.blur3.compat.Blur3Hash;
import org.mytonwallet.app_air.blur3.utils.RenderNodeEffects;
...
public class DownscaleScrollableNoiseSuppressor {
    ...
    private final int k;
    private boolean plainBlurEnabled;
    ...
    public DownscaleScrollableNoiseSuppressor(boolean simple, boolean allowNoiseSuppress) {
        isLiquidGlassEnabled = Blur3Settings.isLiquidGlassEnabled();
        simpleMode = simple;
        k = isLiquidGlassEnabled || allowNoiseSuppress ? 1 : 8; // 1

        this.allowNoiseSuppress = allowNoiseSuppress;
        resultRenderNodes = new RenderNode[isLiquidGlassEnabled ? 3 : !simpleMode ? 2 : 1];
    ...
    /**
     * Glass mode builds the strong blur from the saturated glass node, so an unsaturated result
     * needs its own downscale pass; it is only recorded once a consumer asks for it.
     */
    public void setPlainBlurEnabled(boolean enabled) {
        if (plainBlurEnabled == enabled) {
            return;
        }
        plainBlurEnabled = enabled;
        lastHash = 0;
        for (int a = 0; a < rectRenderNodes.size(); a++) {
            rectRenderNodes.get(a).lastHash = 0;
        }
    }
    ...
    public void draw(Canvas canvas, int index) {
        ...
        if (index == DRAW_GLASS) {
            ...
            canvas.drawRenderNode(resultRenderNodes[isLiquidGlassEnabled ? 2 : 0]);
    ...
    public class DownscaledRenderNode {
        ...
        public void invalidateRenderNodes(RenderNode renderNode) {
            ...
            long hash = 0;
            hash = Blur3Hash.calcHash(hash, renderNode.getUniqueId());
            hash = Blur3Hash.calcHash(hash, downsampledWidth);
            hash = Blur3Hash.calcHash(hash, downsampledHeight);
            hash = Blur3Hash.calcHash(hash, originalWidth);
            hash = Blur3Hash.calcHash(hash, originalHeight);
            ...
            for (int a = 0; a < renderNodeDownsampled.length; a++) {
                ...
                if (simpleMode) {
                    ...
                    renderNodeRestored[a].setPosition(0, 0, originalWidth * k, originalHeight * k);
                    canvas = renderNodeRestored[a].beginRecording(originalWidth * k, originalHeight * k);
    ...
    public void onScrolled(float dx, float dy) {
        for (int a = 0; a < rectRenderNodesCount; a++) {
            ...
            if (sourcePart.renderNodesForPlain != null) {
                sourcePart.renderNodesForPlain.onScrolled(dx, dy);
            }
    ...
    /**
     * Glass Mode: [0] - weak blur, saturation up.      [1] - strong blur over [0], saturation up.
     *             [2] - strong blur, no color matrix (only while plain blur is enabled)
    ...
    private boolean invalidateResultRenderNodes(int width, int height) {
        ...
        hash = Blur3Hash.calcHash(hash, width);
        hash = Blur3Hash.calcHash(hash, height);
        ...
        for (int a = 0; a < resultRenderNodes.length; a++) {
            ...
            for (int b = 0; b < rectRenderNodesCount; b++) {
                ...
                hash = Blur3Hash.calcHash(hash, sourcePart.position.left);
                hash = Blur3Hash.calcHash(hash, sourcePart.position.top);
                hash = Blur3Hash.calcHash(hash, sourcePart.position.right);
                hash = Blur3Hash.calcHash(hash, sourcePart.position.bottom);
                hash = Blur3Hash.calcHash(hash, rn.getUniqueId());
            }
    ...
    private RenderNode getRenderNode(int a, int b) {
        ...
        if (isLiquidGlassEnabled && sourcePart.renderNodesForGlass != null) {
            if (a == 0) {
                ...
                return (sourcePart.renderNodesForPlain.renderNodeRestored[0]);
            }
        } else {
    ...
    private class SourcePart {
        ...
        final @Nullable DownscaledRenderNode renderNodesForPlain;
        ...
        private SourcePart() {
            if (isLiquidGlassEnabled) {
                ...
                renderNodesForPlain = new DownscaledRenderNode("plain", 0);
                renderNodesForPlain.setScale(8, 8);
                renderNodesForPlain.setPrimaryEffectBlur(dpf2(40));
            } else if (simpleMode) {
                renderNodesForBlur = new DownscaledRenderNode("blur", 0);
        ...
        public void invalidate() {
            if (renderNodesForGlass != null) {
                ...
                if (plainBlurEnabled && renderNodesForPlain != null) {
                    renderNodesForPlain.invalidateRenderNodes(renderNode);
                }
            } else {
    ...
    public void drawDebugPositions(Canvas canvas) {
        for (int b = 0; b < rectRenderNodesCount; b++) {
            final SourcePart sourcePart = rectRenderNodes.get(b);
            canvas.drawRect(sourcePart.position, Blur3Theme.DEBUG_GREEN_STROKE);
        }
    }
}
