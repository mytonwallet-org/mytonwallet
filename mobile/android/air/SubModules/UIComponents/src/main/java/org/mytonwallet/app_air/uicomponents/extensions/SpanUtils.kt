...
fun SpannableStringBuilder.updateDotsTypeface(startIndex: Int = 0) {
    ...
    if (index > -1) {
        setDotsBoldSpan(index)
    }
}

fun SpannableStringBuilder.updateDotsLetterSpacing(letterSpacing: Float, startIndex: Int = 0) {
    val index = indexOf(dots, startIndex = startIndex)
    if (index > -1) {
        setDotsLetterSpacingSpan(letterSpacing, index)
    }
}

private fun SpannableStringBuilder.setDotsBoldSpan(index: Int) {
    setSpan(
...
private fun SpannableStringBuilder.setDotsLetterSpacingSpan(letterSpacing: Float, index: Int) {
    setSpan(
        WLetterSpacingSpan(letterSpacing),
        index,
        index + dots.length,
        Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
    )
