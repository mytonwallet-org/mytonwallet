...
data class MStakingStateResponse(val backendState: MStakingBackendState, val state: MStakingState)
...
@JsonClass(generateAdapter = true)
data class MNominatorsPool(val address: String, val apy: Double, val start: Long?, val end: Long?)

// History
@JsonClass(generateAdapter = true)
data class MStakeHistoryItem(val timestamp: Long, val profit: String)
