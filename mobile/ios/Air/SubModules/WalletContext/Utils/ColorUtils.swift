...
public extension UIColor {
    convenience init(hex hexString: String, alpha: CGFloat) {
        if let namedColorHex = getNamedCssColorHex(hexString) {
            self.init(hex: namedColorHex, alpha: alpha)
            return
        }
    ...
    convenience init(hex hexString: String) {
        if let namedColorHex = getNamedCssColorHex(hexString) {
            self.init(hex: namedColorHex, alpha: 1)
            return
        }
        let hex = hexString.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int = UInt64()
...
// MARK: - Named CSS colors

private func getNamedCssColorHex(_ name: String) -> String? {
    NAMED_CSS_COLORS[name.lowercased()]
}

private let NAMED_CSS_COLORS: [String: String] = [
    "aliceblue": "#f0f8ff",
    "antiquewhite": "#faebd7",
    "aqua": "#00ffff",
    "aquamarine": "#7fffd4",
