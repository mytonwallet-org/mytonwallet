//
//  WButton.swift
//  UIComponents
//
//  Created by Sina on 3/30/23.
//

import SwiftUI
import WalletContext
import WalletCore


public struct TokenPickerButton: View {

    public enum Style {
        case regular
        case large
    }

    public var style: Style
    public var token: ApiToken?
    public var inBaseCurrency: Bool
    public var onTap: (() -> ())?
    
    public init(token: ApiToken?, inBaseCurrency: Bool, style: Style = .regular, onTap: (() -> Void)?) {
        self.style = style
        self.token = token
        self.inBaseCurrency = inBaseCurrency
        self.onTap = onTap
    }
    
    public var body: some View {
        Button(action: { onTap?() }) {
            HStack(spacing: style == .large ? 9 : 6) {
                if inBaseCurrency {
                    Text(lang("in"))
                        .textStyle(.footnote, scaling: .dynamic)
                        .foregroundStyle(.secondary)
                        .transition(.scale.combined(with: .opacity).combined(with: .offset(x: 20)))
                }
                WUIIconViewToken(
                    token: token,
                    isWalletView: false,
                    showldShowChain: true,
                    size: style == .large ? 28 : 20,
                    chainSize: style == .large ? 12 : 8,
                    chainBorderWidth: style == .large ? 1 : 0.667,
                    chainHorizontalOffset: style == .large ? 2 : 3,
                    chainVerticalOffset: 1
                )
                    .frame(width: style == .large ? 28 : 20, height: style == .large ? 28 : 20)
                    
                HStack(spacing: 2) {
                    Text(token?.symbol ?? lang("Select Token"))
                        .textStyle(
                            .bodyStrong,
                            content: .technical,
                            scaling: .dynamic
                        )
                    
                    if onTap != nil {
                        Image("SendPickToken", bundle: AirBundle)
                            .textStyle(
                                .footnote,
                                content: .technical,
                                scaling: .dynamic
                            )
                            .foregroundStyle(.secondary)
                    }
                }
                
            }
            .padding(.trailing, onTap == nil ? 2 : 0)
            .fixedSize()
            .padding(.leading, style == .large ? 6 : 10)
            .padding(.trailing, style == .large ? 8 : 10)
            .padding(.vertical, style == .large ? 6 : 8)
            .background(Color.air.secondaryFill, in: .capsule)
        }
        .buttonStyle(.plain)
        .padding(.trailing, onTap == nil ? 2 : 0)
        .animation(.snappy, value: inBaseCurrency)
        .animation(.snappy, value: token)
    }
}
