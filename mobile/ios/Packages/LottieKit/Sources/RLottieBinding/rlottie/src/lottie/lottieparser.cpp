...
protected:
    std::vector<std::pair<std::uint32_t, std::uint32_t>> mColorReplacements;
...
int LottieParserImpl::GetIntOrStringNumber()
{
    if (st_ == kHasNumber && v_.IsInt()) {
        int result = v_.GetInt();
        ParseNext();
...
std::shared_ptr<LOTData> LottieParserImpl::parseLayer(bool record)
{
    ...
    while (const char *key = NextObjectKey()) {
        if (0 == strcmp(key, "ty")) { /* Type of layer*/
            ...
            RAPIDJSON_ASSERT(PeekType() == kNumberType ||
                             PeekType() == kStringType);
            layer->mId = GetIntOrStringNumber();
            ...
            RAPIDJSON_ASSERT(PeekType() == kNumberType ||
                             PeekType() == kStringType);
            layer->mParentId = GetIntOrStringNumber();
