...
public class StackedBarsChartController: BaseChartController {
    ...
    public override var detailsValueTextProvider: ChartDetailsValueTextProvider? {
        didSet {
            barsController.detailsValueTextProvider = detailsValueTextProvider
            zoomedBarsController.detailsValueTextProvider = detailsValueTextProvider
        }
    }

    public override var detailsRowSortOrder: ChartDetailsRowSortOrder {
        didSet {
            barsController.detailsRowSortOrder = detailsRowSortOrder
            zoomedBarsController.detailsRowSortOrder = detailsRowSortOrder
        }
    }

    public override var hidesZeroDetailsRows: Bool {
        didSet {
            barsController.hidesZeroDetailsRows = hidesZeroDetailsRows
            zoomedBarsController.hidesZeroDetailsRows = hidesZeroDetailsRows
        }
    }
    ...
    public init(
        ...
        currency: GraphCurrency? = nil,
        drawCurrency:((CGContext, UIColor, CGPoint)->Void)? = nil,
        rate: Double = 1.0,
        smoothEdges: Bool = false
    )  {
        ...
        barsController = BarsComponentController(isZoomed: false,
                                                 mainBarsRenderer: BarChartRenderer(smoothEdges: smoothEdges),
                                                 ...
                                                 previewBarsChartRenderer: BarChartRenderer(smoothEdges: smoothEdges))
        if let currency {
