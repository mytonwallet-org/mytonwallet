...
class BarsComponentController: GeneralChartComponentController {
    ...
    override func chartDetailsViewModel(closestDate: Date, pointIndex: Int, currency: GraphCurrency? = nil, rate: Double = 1.0) -> ChartDetailsViewModel {
        ...
        if detailsValueTextProvider != nil || detailsRowSortOrder != .original || hidesZeroDetailsRows {
            let items = makeDetailsItems(pointIndex: pointIndex)
            let totalItem = makeTotalItem(
                title: strings.total,
            ...
            viewModel.showPrefixes = viewModel.values.contains { value in
            ...
            viewModel.totalValue = ChartDetailsViewModel.Value(id: "total",
                                                               prefix: nil,
