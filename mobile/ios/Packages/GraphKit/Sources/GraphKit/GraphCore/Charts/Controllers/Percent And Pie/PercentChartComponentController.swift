...
class PercentChartComponentController: GeneralChartComponentController {
    ...
    override func chartDetailsViewModel(closestDate: Date, pointIndex: Int, currency: GraphCurrency? = nil, rate: Double = 1.0) -> ChartDetailsViewModel {
        ...
        let prefixes = chartsCollection.chartValues.map { component in
            total > 0 ? PercentConstants.percentValueFormatter.string(from: component.values[pointIndex] / total * 100) : "0%"
        }
        let items = makeDetailsItems(pointIndex: pointIndex, prefixes: prefixes)
        let values = makeDetailsValues(from: items)
        let totalValue = makeDetailsValue(from: makeTotalItem(
            title: strings.total,
