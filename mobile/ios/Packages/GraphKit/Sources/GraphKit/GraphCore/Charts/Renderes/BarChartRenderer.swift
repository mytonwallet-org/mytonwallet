...
class BarChartRenderer: BaseChartRenderer {
    ...
    private var smoothEdges = false
    
    init(step: Bool = false, lineWidth: CGFloat = 2.0, smoothEdges: Bool = false) {
        self.step = step
        ...
        if var barIndex = bars.locations.firstIndex(where: { $0 >= range.lowerBound }) {
            if fillToTop {
                ...
                if self.step {
                    ...
                    let smoothPaths = makeSmoothAreaPaths(chartFrame: chartFrame, range: range)
                    let colorOffset = Double((1.0 - (1.0 - generalUnselectedAlpha) * selectedIndexAnimator.current) * chartsAlpha)

                    fillSmoothAreaPaths(smoothPaths, context: context, colorOffset: colorOffset)

                    if
                        let selectedBarIndex,
                        selectedIndexAnimator.current > 0,
                        selectedBarIndex < bars.locations.count
                    {
                        let currentLocation = bars.locations[selectedBarIndex]
                        let right = transform(toChartCoordinateHorizontal: currentLocation, chartFrame: chartFrame)
                        let left = transform(toChartCoordinateHorizontal: currentLocation - bars.barWidth, chartFrame: chartFrame)

                        context.saveGState()
                        context.clip(to: CGRect(x: left, y: chartFrame.minY, width: right - left, height: chartFrame.height))
                        fillSmoothAreaSelectionPaths(
                            smoothPaths,
                            context: context,
                            alpha: selectedIndexAnimator.current * chartsAlpha
                        )
                        context.restoreGState()
                    }
                } else {
    ...
    private func makeSmoothAreaPaths(
        chartFrame: CGRect,
        range: ClosedRange<CGFloat>
    ) -> [CGMutablePath] {
        let paths: [CGMutablePath] = bars.components.map { _ in CGMutablePath() }
        guard !bars.components.isEmpty else {
            return paths
        }

        var vertices: [CGFloat] = Array(repeating: 0.0, count: bars.components.count)

        guard var locationIndex = bars.locations.firstIndex(where: { $0 > range.lowerBound }) else {
            return paths
        }

        locationIndex = max(0, locationIndex - 1)
        var currentLocation = transform(toChartCoordinateHorizontal: bars.locations[locationIndex], chartFrame: chartFrame)
        ...
        while locationIndex < bars.locations.count {
            currentLocation = transform(toChartCoordinateHorizontal: bars.locations[locationIndex], chartFrame: chartFrame)

            var stackedValue: CGFloat = 0.0
            for (index, component) in bars.components.enumerated() {
                let visibilityPercent = componentsAnimators[index].current
                let value = component.values[locationIndex] * visibilityPercent
                stackedValue += value
                vertices[index] = stackedValue
            }
            ...
            for (index, value) in vertices.enumerated() {
                paths[index].addLine(
                    to: CGPoint(
                        x: currentLocation,
                        y: transform(toChartCoordinateVertical: value, chartFrame: chartFrame)
                    )
                )
            }

            if bars.locations[locationIndex] > range.upperBound {
                break
            }
    ...
    private func fillSmoothAreaPaths(
        _ paths: [CGMutablePath],
        context: CGContext,
        colorOffset: Double
    ) {
        for (index, path) in paths.enumerated().reversed() {
            ...
            context.setFillColor(
                GColor.valueBetween(
                    start: backgroundColorAnimator.current.color,
                    end: bars.components[index].color,
                    offset: colorOffset
                ).cgColor
            )
            context.fillPath()
    ...
    private func fillSmoothAreaSelectionPaths(
        _ paths: [CGMutablePath],
        context: CGContext,
        alpha: CGFloat
    ) {
        for (index, path) in paths.enumerated().reversed() {
            ...
            context.addPath(path)
            context.setFillColor(
                bars.components[index].color
                    .withAlphaComponent(bars.components[index].color.alphaValue * alpha)
                    .cgColor
            )
            context.fillPath()
            context.restoreGState()
