...
class GeneralChartComponentController: ChartThemeContainer {
    ...
    var detailsRowSortOrder: ChartDetailsRowSortOrder = .original
    var hidesZeroDetailsRows = false
    ...
    func formattedDetailsDateTitle(for date: Date, includesTime: Bool) -> String {
        strings.formattedSingleDateTitle(date, includesTime: includesTime)
    }

    func formattedRangeTitle(from fromDate: Date, to toDate: Date) -> String {
        strings.formattedRangeTitle(from: fromDate, to: toDate)
    }
...
// MARK: - Helpers
    ...
    func axisLabel(for date: Date, scaleType: ChartScaleType) -> String {
        switch scaleType {
        case .day:
            return strings.formattedAxisDate(date)
        case .hour, .minutes5:
            return strings.formattedAxisTime(date)
        }
    }
    ...
    func chartDetailsViewModel(closestDate: Date, pointIndex: Int, currency: GraphCurrency? = nil, rate: Double = 1.0) -> ChartDetailsViewModel {
        let items = makeDetailsItems(pointIndex: pointIndex)
        var values = makeDetailsValues(from: items)
        let totalValue: ChartDetailsViewModel.Value? = nil
        ...
        if let currency, let firstValue = values.first, let starColor = GColor(hexString: "#dda747") {
            ...
            values.append(ChartDetailsViewModel.Value(
                id: "series-converted-\(currency.rawValue)",
                prefix: nil,
        ...
        let dateString = formattedDetailsDateTitle(for: closestDate, includesTime: isZoomed)
    ...
    func makeDetailsItems(pointIndex: Int, prefixes: [String?]? = nil) -> [ChartDetailsItemContext] {
        chartsCollection.chartValues.enumerated().map { arg in
            ...
            return ChartDetailsItemContext(
                ...
                role: .series,
                prefix: {
                    guard let prefixes, prefixes.indices.contains(index) else {
                        return nil
                    }
                ...
                rawValue: rawValue,
                formattedValue: self.detailsNumberFormatter.string(from: NSNumber(value: rawValue)) ?? "",
                color: component.color,
                isVisible: chartVisibility[index]
            )
        }
    }

    func makeTotalItem(title: String, rawValue: Double, color: GColor, isVisible: Bool) -> ChartDetailsItemContext {
        ChartDetailsItemContext(
            id: "total",
            index: Int.max,
            role: .total,
            prefix: nil,
            title: title,
            rawValue: rawValue,
            formattedValue: detailsNumberFormatter.string(from: NSNumber(value: rawValue)) ?? "",
            color: color,
            isVisible: isVisible
        )
    }

    func makeDetailsValue(from item: ChartDetailsItemContext) -> ChartDetailsViewModel.Value {
        let text = detailsValueTextProvider?(item) ?? item.formattedValue
        return ChartDetailsViewModel.Value(
            id: item.id,
            prefix: item.prefix,
            title: item.title,
            value: text,
    ...
    func makeDetailsValues(from items: [ChartDetailsItemContext]) -> [ChartDetailsViewModel.Value] {
        ...
        case .descendingValue:
            orderedItems = preparedItems.sorted { lhs, rhs in
        ...
        return orderedItems.map(makeDetailsValue)
    }
