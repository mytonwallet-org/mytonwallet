...
const RUB_ON_RAMP_CURRENCIES: ApiBaseCurrency[] = ['RUB'];
 ...
 */
export function getOnRampBaselineCurrencies(
  chain?: ApiChain, provider?: 'moonpay' | 'avanchange',
): ApiBaseCurrency[] {
  if (provider === 'moonpay') return ON_RAMP_CURRENCIES_WITHOUT_RUB;
  if (provider === 'avanchange') return chain && getIsRubSupported(chain) ? RUB_ON_RAMP_CURRENCIES : [];
